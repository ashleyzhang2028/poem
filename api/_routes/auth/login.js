 ...
 *      403 { code:"E_EMAIL_UNVERIFIED", emailMask, verifySent, verifyTransport }
 *            ← 口令**对的**、但邮箱没确认（Issue #197 后半段）
 ...
 *      `sendCode` / `verifyCode` 照旧（**只有一处**跟着改了：码验对了但邮箱
 *      没确认时同样回 403 —— 两条路是同一件事的两半，只堵一条等于没堵）。
 *   4. **邮箱没确认就不签发会话**（用户 2026-09-16 裁决）。判据在
 *      `core.emailGate()` 里只写一份，而且**判在口令校验之后** ——
 *      判在前面会把「注册过但没确认」与「压根不存在」分成两种响应，
 *      那就是邮箱枚举。
 *
