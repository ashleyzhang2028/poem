 ...
 *          「现在背诵首页中的自选背诵栏，我觉得应该全部放到设置中去，
 *            首页背诵栏只显示背诵列表，不要干扰背诵。所有导入，导出，
 *            重命名，删除等等应该全部在设置中进行。」
 *          「由于设置项增多，请合理分类」
 *
 *        ① 首页不再有「自选背诵」折叠卡：导入 / 导出 / 改名 / 删除 /
 *           整组移出 / 上移下移整块搬进设置页的「我的清单」组
 *           （js/settings.js，DOM id 与首页那一套同名同构）。
 *           搬走的只是**管理入口** —— 自选篇目照旧跟课内 261 首一起
 *           按遗忘曲线排每日任务，排上了就混在首页今日列表里；
 *           两页读同一份 poem_recite_collections_v1，
 *           在设置里改完回首页，今日任务立刻跟着变。
 *           设置页从此要读站点索引 + 课内 12 册（约 52KB）
 ...
 *           与首页那一枚同一份 CSS）。
 *        （settings/index.html、js/settings.js、js/collections.js、
 *          index.html、js/app.js、css/style.css、test/ui.test.js、
