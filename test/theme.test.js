...
// 需求：页面各个卡片的背景色透明度调整为 95%（留 5% 让祥云纹隐隐透出）
chk(/--card:\s*rgba\(255, 254, 250, \.95\)/.test(css),
  '卡片背景改为 95% 不透明的宣纸白 rgba(255,254,250,.95)');
chk(/\.modal-box \{[\s\S]{0,300}?background:\s*rgba\(252, 250, 243, \.95\)/.test(css),
  '古诗 / 设置弹层同为 95% 不透明');
