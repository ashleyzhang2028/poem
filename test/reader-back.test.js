...
console.log("");
console.log("=== 4 · 列表本来就停在顶部，返回不该乱跳 ===");
{
  const h = harness();
  const { api } = mountBook(h, 60);
  eq(h.win.scrollY, 0, "打开前停在顶部");
  api.open("cs-1");
  api.close();
  h.flushRAF(); h.flushRAF();
  eq(h.win.scrollY, 0, "返回后还是顶部（没有残留的还原把页面推下去）");
}

console.log("");
console.log("=== 5 · 页面自己拆列表时说一声 drop-list，就不要还原 ===");
{
  const h = harness();
  const items = [];
  for (let i = 1; i <= 60; i++) items.push({ id: "cs-" + i, title: "第 " + i + " 条", text: "正文 " + i });
  let leaving = false;
  const api = h.sandbox.window.ReaderEngine.mount({
    id: "changshi", items: items, root: h.root, reader: h.reader, groupOrders: [],
    words: { list: "文学常识", unit: "条" },
    onHideReader: function () { return leaving ? "drop-list" : true; }
  });
  h.win.scrollTo(0, 5000);
  api.open("cs-30");
  api.close();                                    // 留在集子：照样还原
  h.flushRAF(); h.flushRAF();
  eq(h.win.scrollY, 5000, "留在集子：返回照常还原位置");

  api.open("cs-30");
  leaving = true;
  api.close();                                    // 页面要退回书架
  h.flushRAF(); h.flushRAF();
  eq(h.win.scrollY, 0, "页面说要拆列表：不还原，交给下一次进列表");
}

console.log("");
console.log("=== 6 · 顶栏返回键（用户点的那颗）走的就是这条还原链路 ===");
{
  const h = harness();
  const { api } = mountBook(h, 60);
  let action = null;
  h.sandbox.window.SiteChrome = { setHeaderAction: a => { action = a; }, setSub: () => {} };
  h.win.scrollTo(0, 3000);
  api.open("cs-20");
  chk(!!action && typeof action.onclick === "function", "打开详情后顶栏挂上了返回键");
  chk(/返回/.test(action.label), "返回键文案是「" + (action && action.label) + "」");

  action.onclick();                               // 等价于点右上角返回
  h.flushRAF(); h.flushRAF();
  eq(h.win.scrollY, 3000, "点返回键落回 3000（用户报的正是这一步）");
  api.close();                                    // 收尾，避免残留
  eq(api.isOpen(), false, "读者态已收起");
}
