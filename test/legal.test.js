...
setTimeout(() => {
  /* --- 用户协议 --- */
  {
    ...
    chk(/不删(背诵)?进度|都不会删掉背诵进度/.test(t), '写明退出/注销不删背诵进度（进度与账号是两回事）');
    chk(/不是付费凭据/.test(t), '写明本机层级**不是付费凭据**（不许让它看起来像收费凭据）');
    chk(/不收款|无支付入口|没有任何支付入口/.test(t), '写明本应用不收款、无支付入口');
    // 需求 2：协议要保持精简，不再长篇大论
    //
    // ⚠️ 这条上限**上调过一次（1400 → 1500）**，理由不是「写不下了」：
    //    /login/ /profile/ /admin/ 三张页落地之后，协议里多出了两件**必须**
    //    如实写明的事 ——「账号可退出/注销、注销不删进度」与「Free/Pro/Max
    //    是本机登记的功能标记、不是付费凭据、本应用不收款」。
    //    按用户立的「条款永远跟随代码」原则，这两句不许省：省掉就等于
    //    站上出现了「有账号、有层级却只字不提」的空白，比多 70 个字糟得多。
    //    上限仍然卡在 1500 —— 它是「一屏读得完」的代理指标，不是目的本身。
    chk(t.length < 1500, '用户协议精简到一屏可读完（' + t.length + ' 字）');
    // 需求：删掉顶部「一句话版本」速览块
  ...
  /* --- 隐私条款 --- */
  {
    ...
    // 层级也只在本机（新增 /profile/ 的层级徽章与 /admin/ 的发放名单之后）
    chk(/层级|名额/.test(t), '隐私条款提到了层级/名额（新增了订阅层级这件事）');
    chk(/本机/.test(t) && /不上传/.test(t), '层级同样写明只存在本机、不上传');
