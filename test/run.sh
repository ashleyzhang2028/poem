...
#   3g2.乐府诗选测试     —— jsdom + vm，15 首两汉至南北朝乐府 + 三组分法 + 阅读器
#   3g3.近现代诗词测试   —— jsdom + vm，24 首近百年志士之诗 + 分期分组 + 阅读器
...
#                          那条落点（「我的」页账号卡里那颗）与返回落点
#                          （登录 → 「我的」页）（Issue #132）；
#                          Issue #209 之后顶栏那枚印撤了、Issue #244 之后
#                          个人中心整页也删了 —— 现在只剩这一条，更不能断
#   3t4.个人中心已删除  —— 纯 Node + 源码扫描，Issue #244 那一轮：/profile/
#                          那一页整页删掉（页面 / js/profile.js / 预缓存 / 路由 /
#                          全站落点一处不剩），它**独有**的那四件（法务两条入口、
#                          层级对比、跨设备同步开关 + 冲突裁决、管理后台）
#                          真的搬进了「我的」页的「关于」卡 —— 删掉的东西不能
#                          跟着一起消失（正面断言与负面断言各守一半）
#   3x. 账号接线        —— 纯 Node + 假 fetch，2 期「补洞 + 2A」接上的两根线：
#                          /api/me 下发的层级与角色真的落到权益层（服务端优先）、
#                          注销时「先服务端、后本机」且云端那一份导出给用户、
#                          四条边界（失败不打断 / 不假装 / 不清数据 / 不发无谓请求）
#   3t. 账号两页        —— 纯 Node + jsdom，/login/ /admin/ 的结构与口径：
#                          权限判断只走 Entitlement、页面不自己拼 plan、
#                          如实标注「本地体验版」（不许假装有服务器）、
#                          退出只清会话 / 注销不删进度、账号页都不写进度键（Issue #132）；
#                          （Issue #244 之前这里还有 /profile/ 那一页 —— 个人中心
#                           并进「我的」页后删掉了，那一页独有的几件有一条
#                           专门的守卫见下面 3t4）
...
echo ""
echo "=== 乐府集（2026-09 · 课外阅读第二部集子，Issue #244）==="
echo "    精选 100 首，自汉魏至唐五代按卷一至卷五分卷；"
echo "    与课内 / 唐诗重篇的那几十首（《木兰诗》《观沧海》《将进酒》……）"
echo "    走同一条判重与正文收归的路，正文只在存储主表落一份。"
node test/yuefu.test.js
...
echo ""
echo "=== 乐府诗选（2026-09 · 课外阅读第八部集子）==="
echo "    十五首两汉至南北朝的乐府民歌，按汉乐府 / 北朝乐府 / 南朝乐府分组；"
echo "    与课内同篇的那四首（《长歌行》《迢迢牵牛星》《木兰诗》《敕勒歌》）"
echo "    走同一条判重与正文收归的路。"
node test/yuefu.test.js

echo ""
echo "=== 近现代诗词（2026-09 · 课外阅读第九部集子）==="
echo "    二十四首近百年间的志士之诗，按革命历程分六期；原作引用、译文自拟，"
echo "    仅供背诵学习。四首与课内同篇（《沁园春·长沙》《七律·长征》"
echo "    《沁园春·雪》《卜算子·咏梅》），走同一条判重与正文收归的路。"
node test/jinxiandai.test.js
...
echo ""
echo "=== 页面归位 / 文案精简（Issue #278 · 哪些该在「我的」、哪些该在设置）==="
echo "    用户 2026-09-21：「再次梳理所有页面……哪些应该放到我的却放到了设置」+"
echo "    「各项用户提示，能省则省，能删则删，不要废话太多」。守四条结构："
echo "    ① 账号与跨设备同步只在「我的」页（设置侧不许有第二份）；"
echo "    ② 同一件事只有一个入口（层级对比 / 法务 / 自检 / 我的报告各一处）；"
echo "    ③ 选中态自己就是当前值，不再回显一行「当前：…」；"
echo "    ④ 讲「另一个页面在哪」时地址要能点（不做口头指路）。"
node test/page-copy.test.js
...
echo ""
echo "=== 账号两页（/login/ /admin/，Issue #132 + #197 完整登录流程）==="
node test/account-pages.test.js

echo ""
echo "=== 个人中心已删除守卫（Issue #244：那一页独有的四件都搬进「我的」页了）==="
node test/profile-removed.test.js
...
echo ""
echo "=== 账号入口动线（「我的」页那颗账号入口，Issue #132）==="
...
echo ""
echo "=== 人机校验的方框（Issue #225 后续 · 看不见方框时不许再让人去勾它）==="
echo "    用户 2026-09-19：三个环境变量都在托管平台上配好了，页面上却看不见方框，"
echo "    还被一句「请先完成人机校验（上面那个方框）」卡住出不去。"
echo "    守的是那一步的两种坏法分得开："
echo "    ① 「没勾」（方框真在）才许说「请先完成人机校验」；"
echo "    ② widget 渲染失败 / 脚本没加载 = **方框不存在**，这时说真原因"
echo "      （Site Key 填错 / 域名不在允许列表 / 被拦截），并指一条能走的路；"
echo "    ③ 没配就是没配（如实降级），不许报成「坏了」。"
...
echo ""
echo "=== 用户报告 / 勘误（Issue #243 第四轮 · 纯 Node）==="
echo "    守的是用户那句「允许用户报告错误，勘误……我作为管理员能在管理端看到并纠正」："
echo "    详情页那颗小旗（排在译文与加入背诵之间，**没有**状态）、列表行最左那一颗、"
echo "    正文选中后的「报这一段」、设置「关于」里的「我的报告」、管理端那一张台账；"
echo "    报告落到**自己的表**（不进 progress、不参与同步、不按孩子分家）；"
echo "    「谁报的」有快照（账号注销后仍认得出）；管理端两道闸（会话 + 角色）；"
echo "    每日有上限（20 条）与设备档频控；未登录时**给出别的出路**而不是 401 一句；"
echo "    前后端那组上界逐字一致（前端放行、服务端截断是最坏的一种不一致）。"
...
echo ""
echo "=== 注音勘误测试（Issue #243 · 某一篇的某一处读错，谁来兜）==="
echo "    注音是按字查表 + 按词消歧**算**出来的，古文里总有表与词表都没料到的组合
    （《滕王阁序》「秋水共长天一色」的「长」读成了 zhǎng —— 词表里没有「长天」）。
    补词表补不完、且每补一次要发一次版，所以开一层**勘误表**：
    「某一篇 · 某一句 · 第几个字 → 读什么」，管理页面里增删、随账号同步。
    这一层守三条边界：按篇绑定（别篇不受影响）、按位移定位（同一句里两个
    相同的字各读各的）、不传 wid 时输出与改前**逐字相同**（纯增强）。
    另守服务端白名单：半条勘误整条丢掉（收下也不会生效，只会让人以为钉过了）。"
node test/pinyin-fix.test.js
