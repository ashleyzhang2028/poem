...
#   3z2.子档案          —— 纯 Node（Issue #159）：一个家长多个孩子各背各的 ——
#                          名册（数组 + 当前选中项）、上限三档（Free 1 / Pro 3 / Max 180，
#                          按已发放层级判、拿不到内核时不误拦）、**键映射只有一处**
#                          （序号 0 = 无后缀的老键 → 老用户零迁移零感知）、
#                          进度 / 设置 / 已读分家而设备域一个字节不动、
#                          老昵称与老印认领进第一个档案、删除时后面的键**跟着前移**
#                          （不前移的症状不是报错，是「删掉老大之后老二的进度看着空了」）
...
echo ""
echo "=== 子档案（Issue #159 · 一个家长多个孩子各背各的）==="
