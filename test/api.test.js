...
async function main() {
     ...
     ================================================================== */
  {
    ...
    try {
      ...
      // ⚠️ 措辞跟着源码走：api/_lib/core.js 现在回的是
      //    「这一项**免费、不限次**，不计额度（本站不收款，也没有计费）。」
      //    上一轮把口径从「3 期还没有定价」改成「本站不收款」之后，
      //    这条断言仍在找 `不计入额度|定价` —— 量的是旧词，成了假红。
      //    这里守住**意思**（note 说明了为什么不计数、不假装扣费），不钉具体措辞。
      chk(/不计额度|不计入额度|免费|不收款|定价/.test(pro1.body.note),
