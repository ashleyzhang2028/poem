...

{
  ...
  // ---- Issue #209：用户对比 / 用户协议 / 隐私条款 三条链接落在「关于」那几行里 ----
  //
  ...
  chk(/用户对比/.test(aboutBlock) && /\/plans\//.test(aboutBlock),
    '「关于」里有「用户对比」这一行，落在 /plans/（它从「我的」页首卡挪到了这儿）');
  ...
  chk(rowsHtml.indexOf('离线缓存') < rowsHtml.indexOf('/plans/') &&
    ...
    '「用户对比」那一行插在离线缓存与用户协议**中间那一行**（用户点名的位置）');
  // 这几行的条目都是 link(href, 词) 造出来的 —— 左列就是那个 <a>，
  // 不另挂「查看」；右列（kvRow 的第二格）是空串。
  chk(/link\("\/plans\/",\s*"用户对比"\)/.test(rowsHtml),
