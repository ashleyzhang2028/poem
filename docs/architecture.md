...
| 8 | 后端全挂时前端仍能背 | ⚠️ | **层次如实说明**：1A 只落「接口不 500 + 传输层降级」；「同步失败不打断背诵」的端到端测试属 1B（那时才有同步层）。见 §4.7 ③ |
...
- **第二层（属 1B）**：「同步失败不打断背诵」的端到端验证。
  1A 还没有同步层，写一条「同步挂了照样背」的测试等于测一个空对象 ——
  那种断言永远绿、什么都不验，比不写更糟。**所以这里如实标注为 1B 的活。**
