...
`sw.js` 里**不写任何注释**（原因见 `README.md` 的「`sw.js` 禁止注释」一节：
顶部注释块漏一个 `*/` 就曾让整份文件解析失败、Service Worker 从不注册、
离线能力静默失效，只有断网时才发现）。原先待在那个注释块里的**维护约定**
与**逐版沿革**，以及维护约定里那两条硬规矩，都搬到这里。

`sw.js` 只留代码。约定本身仍然有效，只是不再以注释形式待在代码里。

## 一、静态资源是「缓存优先」，改了文件必须升版本

`sw.js` 里 css / js / 字体一律**缓存优先** —— 命中缓存就直接返回，不回源比对。
因此**只要动了 `css/` 或 `js/` 里的任何文件，就必须把 `CACHE_NAME` 的版本号 +1**，
否则老用户会一直拿着旧副本：表现为「明明改了样式，刷新后还是老样子」，改多少遍都不生效。
（`activate` 里会删掉所有非当前版本的缓存，升版本即完成替换。）

- 改了哪些文件就升一次，宁可多升，不要漏升。
- 版本回滚同理：版本号只能往上走，不要改回旧号，否则老缓存会被复用。
- 同时有几个 PR 在改 `css/` 或 `js/` 时，**每次 +5、不要只 +1**：
  各自从同一基线 +1 会撞在同一个版本号上，先合的那个把号占了，后面的每个 PR
  都得回来解一次冲突。+5 留出余量，各自落在不同号段上。
  （号段之间留空不碍事：版本号只用来区分新旧，不要求连续。）

## 二、`sw.js` 禁止注释

`sw.js` 不含任何注释（`//` 行注释与 `/* */` 块注释都不写）。
版本沿革（哪一版改了什么）在本文件末尾的「版本历史」里，
面向使用者的说明在 `README.md` 里。

## 三、这个 Service Worker 做什么
...
`test/theme.test.js` 有三条防线：

- `node --check sw.js` 保证它能解析（注释块漏 `*/` 那类错误直接判红）；
- 一条源码扫描断言「`sw.js` 里不出现任何注释」（用户要求：去除并禁止再加）；
- 一条「缓存版本号 ≥ 下界、且 `CACHE_NAME` 是带版本号的常量」；
- 以及一条「缓存优先、改动需升级版本」的约定断言 —— 它读的是**本文件**，
  不再读 `sw.js` 的注释。
...
```
  ...
  v103 sw.js 去掉全部注释，并禁止再加（Issue #114 · 用户要求）：
       ...
       那块注释曾漏一个 `*/`，整份文件解析报 SyntaxError、Service Worker
       从不注册、离线能力静默失效（只有断网时才发现）。既然注释是这条坑的
       唯一来源，就直接禁掉：
         · sw.js 只留代码（147 行），不含任何 `//` 或 `/* */`；
         · 维护约定（缓存优先 / 改动升版本 / 并行 PR 每次 +5）移到本文件；
         · 逐版沿革移到本文件的「版本历史」（SW-NOTE.md + docs/sw-history.md
           合并为这一份 SW-NOTE.md，不留两份各说各话）；
         · test/theme.test.js 加一条源码扫描断言「sw.js 里不出现任何注释」，
           并把原先「读 sw.js 注释验缓存优先约定」那条改读 SW-NOTE.md；
         · README.md 补「sw.js 禁止注释」一节，并把指向 sw.js 注释的
           那句「这条约定写在 sw.js 顶部」改到 SW-NOTE.md。
       版本号按「并行 PR 一律 +5」的口径从 v98 起落到 v103。
       （sw.js、SW-NOTE.md、README.md、test/theme.test.js）
       【并入 main 时解冲突】main 那笔 v93（自选背诵搬进设置页）与
       v88（课内诗词索引页）是**注释里的版本沿革**，随 sw.js 注释块一并
       搬进本文件；sw.js 本身只留代码，CACHE_NAME 取本条的 v103
       （main 为 v98，本条按 +5 口径落 v103）。
  ...
  v88  收掉 #119 遗留的两处「点下去没反应 / 两处数对不上」（Issue #114 后续）：

       【一】首页接上 `/?poem=<id>` 的落地 —— 断了的那一头。
       /progress/ 那份「全部到期篇目」的篇名是指回首页的链接
       （`/?poem=<id>`），链接形态本身有测试守着；但**全站没有一处读它**
       （js/app.js / js/chrome.js / js/reader-core.js 都搜不到
       location.search）—— 点过去首页照常画「今日背诵」，什么也不弹。
       不报错、测试全过，用户只觉得「没反应」，是最难查的那种坏。
       现在 js/app.js 补上落地：认 `?poem=`（并兜底认 hash），
       课内 261 首与自选集合里的课外篇目都能打开，
       走的是**同一个详情弹层**（正文 / 译文 / 朗读 / 三个掌握度按钮照旧）；
       读完就用 replaceState 把地址栏那一截抹掉 —— 刷新不再弹、
       返回键回得到干净的首页；查不到的 id 安静放过（旧链接不该弹黄条）。
       自选篇目沿用 ReciteCollections.scheduleItems() 这一个出口
       （与自选列表点开同一条路，缺新语料时自动回落到加入时的快照）。
       （js/app.js、test/ui.test.js）

       【二】「今天 N 篇」与日历那一格对齐 —— #119 的真 bug。
       renderDueList 的汇总行原先写的是
       `L.days[0].items.length + L.backlog.length`（把「逾期超过一周」
       那一批也并进了今天），而日历上「今天」那一格与清单今天那一档的表头
       都**不含**它们（overview 的 overdue 是另计一条）——
       于是只要有一篇逾期超过一周，日历说今天 1 篇、清单汇总说 2 篇，
       正是「同一本账」要防的情形。
       现在汇总行拆成三截各说一件事：今天 N 篇（= 日历今天那一格）·
       逾期超过一周 M 篇（另计）· 未来两周共 T 篇；
       两处顺带纠了两句写反的文案（「逾期超过一周的已并进今天」→
       实为**另计**，不在今天那一格里）。
       test/progress.test.js 造了「正好逾期 7 天 / 逾期 8 天 / 逾期 9 天」
       的边界数据逐格对账（原先只造到「逾期 2 天」，绕过了这个边界），
       页面层也补了「今天那一档表头 = 日历今天那一格」的正面断言。
