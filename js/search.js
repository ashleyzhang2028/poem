...
(function () {
  ...
  /* ---------------- 搜索框的位置 ----------------
   *
   * 用户这一轮的要求：**聚焦时把搜索框向上挪到标题栏下方**（下拉跟着上去）；
   * 点空白地方时下拉消失；**有搜索内容时搜索框停在页面顶部**；
   * **清空内容且不在焦点上时回到页面中心**。
   *
   * 这四句合起来是一个「跟着状态走」的位置：打字 / 有结果时贴着顶栏（一屏里
   ...
   *   · 点在**结果区**上 → 交给 bindSuggestDismiss（先收下拉，并且吃掉这一下，
   *     不让它穿过浮层开一篇）；
   *   · 点在别处 → blur 已经会收（延迟 180ms，给鼠标点候选留时间），
   *     这里不再重复收一遍，免得与那条定时器打架。
   * 所以这一条**只需要管「点空白处时页面不跳」**：把所有非输入框区域的处理
   * 交给浏览器默认（不 preventDefault），保持点击目标语义干净。
   */
