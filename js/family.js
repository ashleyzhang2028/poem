/**
 * 家庭子用户 · 纯逻辑内核（3 期 P1 · `profile.family`）
 * ==========================================================================
 ...
 *    那份昵称 + 字符印会在第一次读的时候被**认领成第一个子用户**，
 *    昵称与印一个字都不丢。认领是幂等的，且**只在子用户表为空时**发生。
 *
 * ## 语法与依赖
 *
...
(function (root, factory) {
  ...
  /** 账号域：子用户表（跨设备一致 —— 与 `poem_profile_v1` 同一个域） */
  var NS = "poem_family_v1";
  /** 老键：0 期至今那一份「只有昵称 + 头像」的档案，认领成第一个子用户 */
   ...
   *      会被判成 free —— 症状是「昨天有 3 个子用户，今天一刷新只剩 1 个」。
   *    · 拿不到权益内核时**不设限**（返回 Infinity）：脚本顺序不对 / 老缓存时
   *      按 free 卡，症状是一个由加载顺序引起、用户无法自查的功能倒退。
   *
   * 层级对应（用户 2026-09-17 裁决「子用户 Max 180 个」）：
  ...
  /** 一条子用户归一化；`id` 缺失就现发一个（脏数据也要能救回来，而不是整份丢掉） */
  ...
  /**
   * 认领老档案时**顺手把老数据也搬到第一个子用户名下**。
   *
   * 不做这一步的症状是：老用户升级之后，昵称还在，但**进度空了** ——
   * 因为多子用户一开启，盘上就读 `..._v1::f-xxx` 这把新键，
  ...
  /** 切换当前子用户。不存在的 id 一律拒绝（不回落到第一条 —— 那是静默改行为） */
  ...
  /** 当前子用户 id（没认领过老档案时给空串 —— 「还没有档案」是如实的状态） */
   ...
   *
   * 返回的键名 **不含** 子用户 id —— 那是 `keyFor()` 的事（拼法只有一处）。
   */
  function sharedKeys() {
    var g = typeof globalThis !== "undefined" ? globalThis : null;
   ...
   *
   * ⚠️ 没有当前子用户（空串）时**返回原键**：0 期至今那几把键的形状
