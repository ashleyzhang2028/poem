 ...
 *
 * ## 子用户（Issue #159 · 用户 2026-09-17 裁「进度也分家」「Free 档给 1 个昵称」）
 *
 ...
 *
 * ## 家庭子用户（3 期 P1 · `profile.family`）：哪几把键跟着孩子分家
 *
 * `js/family.js` 持有「一个家长多个小孩」的全部逻辑（档案名册 + 当前选中 + 上限）。
 * 这一层只做一件事：**读 / 写盘的键名按当前子用户拼一遍** ——
 *
 * | 键 | 跟着孩子分家吗 | 为什么 |
 * |---|---|---|
 * | 进度域 `poem_recite_progress_v1` | ✅ | 这就是子用户存在的理由：各背各的 |
 ...
 * ⚠️ **没有子用户时（老用户 / 脚本顺序不对）返回原键**：0 期至今盘上形状
...
(function () {
  ...
  var DEFAULTS = {
    ...
    /* ⚠️ 出厂范围从 `term`（本年级本学期）改成 `upto`（本册及之前）——
       Issue #209：界面上删掉了「本年级本学期」那一档，出厂值再留着 `term`
       会让新用户落在一个**界面里选不中**的范围上（首页写着「本年级本学期」，
       设置页却没有那一档）。与 js/scheduler.js 的 DEFAULT_SCOPE 必须同值。 */
    scope: "upto",
    algo: "ebbinghaus"
  };
  ...
  /* ---------------- 家庭子用户：按当前孩子拼键名（唯一的拼接处） ----------------

     这一层**不实现**子用户的逻辑（名册 / 切换 / 上限都在 `js/family.js`），
  ...
  function setHelper(on) {
    ...
    legacy.helper = v;
    put(KEYS.settings, JSON.stringify(legacy));   // 走 physKey：镜像落在**当前子用户**的设置里
    return ok;
  }
  ...
  /* ⚠️ 这一把键**不拼子用户后缀**：scopes 表里它是 `perChild: false` ——
  ...
  function exportJSON() {
    ...
    /* 家庭子用户（3 期 P1）：**整份名册**进备份，不是只导当前那一个。
  ...
  function importJSON(text) {
    ...
    /* 家庭子用户**先导**：后面的进度/设置要落到「当前那个孩子」的键上，
  ...
  /* ⚠️ 绝不能顺手把账号域 / 设备域一起清了 —— 那是「清空背诵进度把昵称和
     ...
     ⚠️ 多子用户之后还有第二条：**默认只清「当前这个孩子」的那一份**。
  ...
  window.ProgressStore = {
    ...
    /* 家庭子用户（3 期 P1）：这一层只提供「当前该读哪把键」的出口，
