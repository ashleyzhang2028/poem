...
- **未登录 ≠ Free，且只差两条**：差的是「语音朗读」与「进度导出」都要登录
  （`minTier: free` + `login: true` 那一档）。单独给它一列，是因为不这么摆的话，
  用户会把「游客不能听 / 不能导」读成「免费版不能听 / 不能导」
- **绿色那颗勾是一枚 SVG，不是字体里的 ✓**：
...
- **要登录的两件（免费档）**：**语音播放**与**进度导出**。未登录（游客）不能听、
  不能导，登录后的 free 一键不少。两道门都只设在**唯一出口**上 ——
